...
signed dkhan_mn_ghyr_kwfta() { fast(); file();
    TEST{
        ll n; cin >> n;
        vector<ll> nah1(n) , nah2(n);
        bool check1 = false;
        bool check2 = false;
        for (ll i = 0; i < n; i++) {
            cin >> nah1[i];
            if (nah1[i] == 1) check2 = true;
        }
        for (ll i = 0; i < n; i++) {
            cin >> nah2[i];
            if ( nah2[i] == 0 ) check1 = true;
        }
 
 
        if ( nah1 == nah2 ) { cout<<0<<endl; continue; }
 
        ll count=0;
        for ( int i = 0; i < n; i++ ) {
            if ( nah1[i] == 1 && nah2[i] == 0 ) count++;
        }
 
        if ( !check1 or !check2 ) cout<<-1<<endl;
        else if ( count%2 == 0 ) cout<<2<<endl;
        else cout<<1<<endl;
    }
}