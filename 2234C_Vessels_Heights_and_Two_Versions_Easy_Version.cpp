...
signed The_king() { fast(); file();
    TEST{
        ...
        ll sum = 0;
        for ( ll i = 0 ; i < n ; i++) {
            vector<ll> l3nat_allh_3ly_math (n,0);
 
            ll rmx = 0;
            for ( int j = 0 ; j < n-1 ; j++ ) {
                ll idx1 = (i + j) % n;
...
 
                lmx = max( lmx , a[idx1] );
                l3nat_allh_3ly_math[idx2] = min( l3nat_allh_3ly_math[idx2], lmx );
            }
 
            ll total = 0;
            for ( ll k = 0 ; k < n ; k++ ) total += l3nat_allh_3ly_math[k];
            ans[i] = total;
        }
 
